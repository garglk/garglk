...
void initCompiler (size_t size)
{
    ...
    assert (sizeof(git_uint32) == sizeof(void*));
    ...
    sTempStart = sTempEnd = (PatchNode*) (gHashTable + sBufferSize);
}
...
Block compile (git_uint32 pc)
{
    ...
    if (i == LONGJMP_NO_ERROR)
    {
        ...
        while (!done)
        {
            ...
            sPatch->isReferenced = sNextInstructionIsReferenced;
    ...
    for ( ; sTempStart < sTempEnd ; ++sTempStart)
    {
        ...
        if (sTempStart->isReferenced)
        {
            ...
            node->next = gHashTable [node->address & (gHashSize-1)];
...
static git_uint32 findCutoffPoint ()
{
    ...
    for (h = start ; h < top ; h = END_OF_BLOCK(h))
    {
        ...
        {
            ++blockCount;
            runCount += h->runCounter >> 16;
        }
    }

    // For now, the cut-off is just half the average run count.
    // (The 16-bit shifts are intended to prevent overflow.)

    return ((runCount / blockCount) << 16) / 2;
}
...
static void compressWithCutoff (git_uint32 cutoff)
{
    ...
    while (h < top)
    {
        ...
        if (h->runCounter >= cutoff && h->glulxSize > 0)
        {
            ...
            memmove (sCodeTop, h, h->compiledSize * sizeof(git_uint32*));
            sCodeTop += h->compiledSize;
            ++saveCount;
        }
        else
        {
...
//    glk_put_string ("[resetting cache]\n");
    ...
    sCodeStart = sCodeTop = (Block) (sBuffer + gHashSize);
