/*
$Header: d:/cvsroot/tads/TADS2/OBJ.H,v 1.3 1999/07/11 00:46:30 MJRoberts Exp $
*/
...
/*
Name
  obj.h - object definitions
Function
  Defines interface for object handling routines
Notes
  An object is divided into two parts:  a "static" part and a "dynamic"
  part.  The static part contains all of the properties originally
  defined in the object; none of these properties can be deleted, but
  they can have a flag set to indicate that they are to be ignored,
  which effectively deletes them.  The dynamic part changes at run-time,
  with properties deleted and changed as requested.
  
  The static part is kept around at all times to allow objects to revert
  back to their original state with very little work.  For TADS games,
  this allows objects to be restored to their pre-play state after game
  play, without requiring a separate snapshot of the objects.  In
  addition, it allows object state to be saved in a very compact format,
  because only the object header and dynamic portion need to be written;
  the static portion, which may contain a large amount of code, need not
  be saved when saving object state.
Modified
...
struct objdef
{
    ushort   objws;                      /o workspace object was defined in o/
    ushort   objflg;                                        /o object flags o/
    ushort   objnsc;                              /o number of superclasses o/
    ushort   objnprop;            /o number of properties defined in object o/
    ushort   objfree;          /o offset of next free byte in property area o/
    ushort   objrst;        /o reset point - offset of end of 'static' part o/
    ushort   objstat;                      /o number of 'static' properties o/
    objnum   objsc[1];                                      /o superclasses o/
};
...
typedef struct objucxdef objucxdef;
