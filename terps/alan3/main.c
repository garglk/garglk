...
static void loadAndCheckMemory(ACodeHeader tmphdr, Aword crc, char err[]) {
    ...
    if (crc != tmphdr.acdcrc) {
        ...
        if (!ignoreErrorOption)
            ...
            printf("%s%s%s", "<WARNING! ", err, " - Ignored, proceed at your own risk.>\n");
        }
    }
}
...
static void alphaRunningLaterGame(char gameState) {
    printf("<WARNING! You are running an alpha interpreter, but the game is generated by a ");
    if (gameState == 'b')
        printf("beta");
    else
        printf("release");
    printf(" state compiler which was released later. This might cause the game to not work fully as intended. Look for an upgraded game file.>\n");
}
