...
static const sc_byte	V400_SEPARATOR[3]	= { 0xBD, 0xD0, 0x00 },
			V390_SEPARATOR[3]	= { 0x2A, 0x2A, 0x00 },
			V380_SEPARATOR[3]	= { 0x2A, 0x2A, 0x00 };
static const sc_uint16	SEPARATOR_SIZE		= 3;
...
/* Enumerations of key element types and property types. */
static const sc_char	KEY_STRING		= 's',
			KEY_INTEGER		= 'i',
			PROP_INTEGER		= 'I',
			PROP_BOOLEAN		= 'B',
			PROP_STRING		= 'S';
...
{ "GLOBAL",	"$GameName $GameAuthor $DontUnderstand #Perspective"
		...
		" BStatusBox $StatusBoxText iUnk1 iUnk2 BEmbedded" },
{ "BATTLE",	"iStaminaLo iStaminaHi iStrengthLo iStrengthHi"
		" iAccuracyLo iAccuracyHi iDefenseLo iDefenseHi"
		" iAgilityLo iAgilityHi iRecovery" },
{ "ROOM",	"$Short $Long"
		" ?GEightPointCompass:[12]<ROOM_EXIT>Exits"
		" ?!GEightPointCompass:[8]<ROOM_EXIT>Exits"
		" <RESOURCE>Res V<ROOM_ALT>Alts"
		" ?!GNoMap:bHideOnMap" },
{ "ROOM_EXIT",	"{V400_ROOM_EXIT:#Dest_#Var1_#Var2_#Var3}" },
{ "ROOM_ALT",	"$M1 #Type <RESOURCE>Res1 $M2 #Var2 <RESOURCE>Res2 #HideObjects"
		" $Changed #Var3 #DisplayRoom" },
...
{ "OBJECT",	"$Prefix $Short V$Alias BStatic $Description"
		...
		" #Openable ?#Openable=5:#Key ?#Openable=6:#Key"
		" ?#Openable=7:#Key #SitLie ?!BStatic:BEdible BReadable"
		" ?BReadable:$ReadText ?!BStatic:BWeapon #CurrentState"
		" ?!#CurrentState=0:$States,BStateListed"
		" BListFlag <RESOURCE>Res1 <RESOURCE>Res2"
		" ?GBattleSystem:<OBJ_BATTLE>Battle"
		" $InRoomDesc #OnlyWhenNotMoved" },
{ "OBJ_BATTLE",	"iProtectionValue iHitValue iMethod iAccuracy" },
...
{ "TASK_RESTR",	"#Type ?#Type=0:#Var1,#Var2,#Var3 ?#Type=1:#Var1,#Var2"
		...
		" ?#Type=4:#Var1,#Var2,#Var3,$Var4 $FailMessage" },
{ "TASK_ACTION","#Type ?#Type=0:#Var1,#Var2,#Var3 ?#Type=1:#Var1,#Var2,#Var3"
		" ?#Type=2:#Var1,#Var2 ?#Type=3:#Var1,#Var2,#Var3,$Expr,#Var5"
		" ?#Type=4:#Var1 ?#Type=5:#Var1,#Var2"
		" ?#Type=6:#Var1,#Var2,#Var3 ?#Type=7:iVar1,iVar2,iVar3" },
...
{ "NPC_BATTLE",	"iAttitude iStaminaLo iStaminaHi"
		" iStrengthLo iStrengthHi iAccuracyLo iAccuracyHi"
		" iDefenseLo iDefenseHi iAgilityLo iAgilityHi iSpeed"
		" iKilledTask iRecovery iStaminaTask" },
{ "TOPIC",	"$Subject $Reply #Task $AltReply" },
{ "WALK",	"#NumStops BLoop #StartTask #CharTask #MeetObject #ObjectTask"
		" #StoppingTask #MeetChar $ChangedDesc {WALK:#Rooms_#Times}" },
...
{ "BATTLE",	"iStamina iStrength iDefense" },
...
{ "OBJ_BATTLE",	"iProtectionValue iHitValue iMethod" },
{ "ROOM_LIST1",	"#Type {ROOM_LIST1}" },
...
{ "TASK_ACTION","#Type |V390_TASK_ACTION:Type>4?#Type++|"
		...
		" ?#Type=4:#Var1"
		" ?#Type=6:#Var1,ZVar2,ZVar3 ?#Type=7:iVar1,iVar2,iVar3" },
{ "ROOM_LIST0",	"#Type {ROOM_LIST0}" },
{ "EVENT",	"$Short #StarterType ?#StarterType=2:#StartTime,#EndTime"
		" ?#StarterType=3:#TaskNum #RestartType BTaskFinished #Time1"
		" #Time2 $StartText $LookText $FinishText <ROOM_LIST0>Where"
...
{ "NPC",	"$Name $Prefix [1]$Alias $Descr #StartRoom $AltText #Task"
		...
		" [4]<RESOURCE>Res"
		" ?GBattleSystem:<NPC_BATTLE>Battle" },
{ "NPC_BATTLE",	"iAttitude iStamina iStrength iDefense iSpeed iKilledTask" },
{ "TOPIC",	"$Subject $Reply #Task $AltReply" },
{ "WALK",	"#NumStops BLoop #StartTask #CharTask #MeetObject #ObjectTask"
		" #StoppingTask ZMeetChar $ChangedDesc {WALK:#Rooms_#Times}" },
{ "ROOM_GROUP",	"$Name {ROOM_GROUP:[]BList}" },
{ "SYNONYM",	"$Replacement $Original" },
...
parse_select_schema (sc_tafref_t taf)
{
	...
	switch (taf_get_version (taf))
	    {
	    case TAF_VERSION_400:	return V400_PARSE_SCHEMA;
	    case TAF_VERSION_390:	return V390_PARSE_SCHEMA;
	    case TAF_VERSION_380:	return V380_PARSE_SCHEMA;
	    default:
...
static sc_uint32		parse_tafline		= 0;
...
static sc_uint16		parse_stack	= 0;
...
{
	/* Check the stack still has space. */
	if (parse_stack == PARSE_MAX_DEPTH)
		sc_fatal ("parse_push_key: stack overrun\n");

	/* Push the key, and it's associated type. */
	parse_vt_key[parse_stack]	= vt_key;
	parse_format[parse_stack]	= type;
	parse_stack++;
}
 ...
 * index.  An expedient fix is to switch i-s keys before storing a
 * property value; note that the caller should take a safe copy of the
 * stack beforehand, and restore it before returning to the parser.
 */
static void
parse_stack_swap (sc_uint16 i, sc_uint16 j)
{
	sc_char		temp;
	sc_vartype_t	vt_temp;

	/* Standard swap of format and keys. */
	temp		= parse_format[i];
	parse_format[i]	= parse_format[j];
	parse_format[j]	= temp;
	vt_temp		= parse_vt_key[i];
	parse_vt_key[i]	= parse_vt_key[j];
	parse_vt_key[j]	= vt_temp;
}

static void
parse_adjust_stack (void)
{
	sc_uint16	index;

	/* Switch index-string key pairs. */
	for (index = 0; index < parse_stack - 1; index++)
	    {
		if (parse_format[index] == KEY_INTEGER
				&& parse_format[index + 1] == KEY_STRING)
		    {
			parse_stack_swap (index, index + 1);
			index++;
		    }
	    }
}
...
{
	sc_vartype_t		safe_parse_vt_key[PARSE_MAX_DEPTH];
	sc_char			safe_parse_format[PARSE_MAX_DEPTH];
	sc_char			format[PARSE_MAX_DEPTH + 4];

	/* Adjust the stack for parse oddness. */
	memcpy (safe_parse_vt_key, parse_vt_key, sizeof (parse_vt_key));
	memcpy (safe_parse_format, parse_format, sizeof (parse_format));
	parse_adjust_stack ();

	/* Create the format for the property put. */
	format[0] = type; format[1] = '-'; format[2] = '>';
	memcpy (format + 3, parse_format, parse_stack);
	format[parse_stack + 3] = NUL;

	/* Store the property under the stacked keys. */
	assert (parse_bundle != NULL);
	prop_put (parse_bundle, format, vt_value, parse_vt_key);

	/* Unadjust the stack back to how we found it. */
	memcpy (parse_vt_key, safe_parse_vt_key, sizeof (parse_vt_key));
	memcpy (parse_format, safe_parse_format, sizeof (parse_format));
}
...
parse_get_property (sc_vartype_t *vt_rvalue, sc_char type)
{
	...
	/* Create the format for the property get. */
	format[0] = type; format[1] = '<'; format[2] = '-';
	memcpy (format + 3, parse_format, parse_stack);
	format[parse_stack + 3] = NUL;
	...
	assert (parse_bundle != NULL);
	status = prop_get (parse_bundle, format, vt_rvalue, parse_vt_key);
...
static sc_char			*parse_pushback_line	= NULL;
...
parse_get_taf_string (void)
{
	...
	if (parse_use_pushback)
		...
		assert (parse_pushback_line != NULL);
		...
		if (line == NULL)
		    {
			sc_error ("parse_get_taf_string: out of TAF data"
					" at line %ld\n", parse_tafline);
...
parse_get_taf_integer (void)
{
	...
	if (sscanf (line, "%ld", &integer) != 1)
	    {
		sc_error ("parse_get_taf_integer: invalid integer"
					" at line %ld\n", parse_tafline - 1);
		longjmp (parse_taf_error, 1);
	    }
...
parse_get_taf_boolean (void)
{
	...
	if (sscanf (line, "%hu", &boolean) != 1)
	    {
		sc_error ("parse_get_taf_boolean: invalid boolean"
					" at line %ld\n", parse_tafline - 1);
		longjmp (parse_taf_error, 1);
	    }
	if (boolean != 0 && boolean != 1)
	    {
		sc_error ("parse_get_taf_boolean: warning: suspect boolean"
					" at line %ld\n", parse_tafline - 1);
	    }
...
enum {	PARSE_INTEGER		= '#',	PARSE_DEFAULT_ZERO	= 'Z',
	PARSE_BOOLEAN		= 'B',
	PARSE_DEFAULT_FALSE	= 'F',	PARSE_DEFAULT_TRUE	= 'T',
	PARSE_STRING		= '$',	PARSE_DEFAULT_EMPTY	= 'E',
	PARSE_MULTILINE		= 'M',
	PARSE_VECTOR		= 'V',	PARSE_VECTOR_ALTERNATE	= 'W',
	PARSE_ARRAY		= '[',
	PARSE_EXPRESSION	= '?',	PARSE_EXPRESSION_NOT	= '!',
	PARSE_GLOBAL_EXPRESSION	= 'G',
	PARSE_CLASS		= '<',	PARSE_FIXUP		= '|',
	PARSE_SPECIAL		= '{',
	PARSE_IGNORE_INTEGER	= 'i',	PARSE_IGNORE_BOOLEAN	= 'b',
	PARSE_IGNORE_STRING	= 's' };

/* Forward declarations of parse functions for recursion. */
static void	parse_element (sc_char *aelement);
static void	parse_class (sc_char *aclass);
static void	parse_descriptor (sc_char *descriptor);
 ...
 */
static void
parse_array (sc_char *array)
{
	sc_uint32		count, index;
	sc_char			*nextp;
	sc_vartype_t		vt_key;
	...
	/* Find the count of elements in the array. */
	if (sscanf (array, "[%lu]", &count) != 1)
		sc_fatal ("parse_array: bad count in [], %s\n", array);

	/* Advance pointer to characters following []. */
	for (nextp = array + 1; isdigit (*nextp); )
		nextp++;
	nextp++;

	/* Parse the remaining part of the array count times, each a key. */
	for (index = 0; index < count; index++)
	   {
		vt_key.integer = index;
		parse_push_key (vt_key, KEY_INTEGER);
 ...
 */
static void
parse_vector (sc_char *vector, sc_bool is_offset)
{
	sc_uint32		count, index;
	sc_vartype_t		vt_key;
	...
	/*
	 * Find the count of elements in the vector, and adjust if this
	 * is a vector described by size - 1.
	 */
 ...
 */
static sc_bool
parse_test_expression (sc_char *texpr)
{
	sc_vartype_t		vt_key;
	...
	switch (texpr[0])
		...
		vt_key.string = texpr + 1;
		parse_push_key (vt_key, KEY_STRING);
...
static void
parse_expression (sc_char *expr)
{
	sc_char		texpr[PARSE_TEMP_LENGTH];
	sc_bool		is_present;
	...
	if (sscanf (expr, "?%[^:]", texpr) != 1)
		sc_fatal ("parse_expression: bad expression, %s\n", expr);

	/* Handle the remainder of the expression only if test passes. */
	is_present = (texpr[0] == PARSE_EXPRESSION_NOT)
					? !parse_test_expression (texpr + 1)
					:  parse_test_expression (texpr);
	if (is_present)
	    {
		sc_char		*nextp;

		/*
		 * Following the ':' may be a single element, or a comma-
		 * separated list.
		 */
		nextp = expr + strlen (texpr) + 2;
		while (*nextp != NUL)
		    {
			sc_char		element[PARSE_TEMP_LENGTH];

			/* Get the next individual element to parse. */
			if (sscanf (nextp, "%[^,]", element) != 1)
			    {
				sc_fatal ("parse_expression:"
						" bad list, %s\n", nextp);
			    }

			/* Parse it, and advance to start of next element. */
			parse_element (element);
			nextp += strlen (element);
			if (*nextp == COMMA)
				nextp++;
		    }
	    }
...
parse_read_multiline (void)
{
	...
	switch (taf_get_version (parse_taf))
	    {
	    case TAF_VERSION_400:	separator = V400_SEPARATOR;	break;
	    case TAF_VERSION_390:	separator = V390_SEPARATOR;	break;
	    case TAF_VERSION_380:	separator = V380_SEPARATOR;	break;
	    default:
	...
	while (memcmp (line, separator, SEPARATOR_SIZE) != 0)
	    {
		mline = sc_realloc (mline, strlen (mline) + strlen (line) + 2);
		strcat (mline, "\n");
		strcat (mline, line);
		line = parse_get_taf_string ();
	    }
 ...
 * parse_terminal()
 *
 * Common handler for string, integer, boolean, and multiline parse
 * terminals.
 */
static void
parse_terminal (sc_char *terminal)
{
	...
	switch (terminal[0])
		...
		break;
	    case PARSE_DEFAULT_FALSE: case PARSE_DEFAULT_TRUE:
		...
		vt_value.string = parse_read_multiline ();
		parse_put_property (vt_value, PROP_STRING);

		assert (parse_bundle != NULL);
		prop_adopt (parse_bundle, vt_value.string);
		break;
...
static const sc_uint32		RESOURCE_GROW_INCREMENT	= 32;
static sc_uint32		parse_resources_length	= 0;
static sc_uint32		parse_resources_size	= 0;
...
parse_clear_v400_resources_table (void)
{
	...
	if (parse_resources != NULL)
	    {
		sc_uint32	index;

		for (index = 0; index < parse_resources_length; index++)
			sc_free (parse_resources[index].name);
 ...
 */
static sc_int32
parse_get_v400_resource_offset (sc_char *name,
		sc_int32 length, sc_int32 *real_length)
{
	sc_char		*clean_name;
	sc_uint32	hash, index;
	sc_int32	offset;

	/*
	 * Take a copy of the name, and remove any trailing "##" looping
	 * sound indicator flag.  Who thinks this junk up?
	 */
	clean_name = sc_malloc (strlen (name) + 1);
	strcpy (clean_name, name);
	if (!strcmp (clean_name + strlen (clean_name) - 2, "##"))
		clean_name[strlen (clean_name) - 2] = NUL;

	/*
	 * Scan the current resources list for a matching name, and if
	 * the resource is already known, return its offset.  The hash check
	 * is an attempt to improve the search times, relative to using only
	 * string comparisons -- the table's not fully hashed.  If found,
	 * we need to also pass back the corrected length.
	 */
	offset = -1;
	hash = sc_hash (clean_name);
	for (index = 0; index < parse_resources_length; index++)
	    {
		if (parse_resources[index].hash == hash
			&& !strcmp (parse_resources[index].name, clean_name))
		    {
			offset = parse_resources[index].offset;
			break;
		    }
	    }
	if (offset != -1)
	    {
		*real_length = parse_resources[index].length;
		sc_free (clean_name);
		return offset;
	    }
	...
	if (parse_resources_length == parse_resources_size)
		...
		parse_resources = sc_realloc (parse_resources,
			parse_resources_size * sizeof (parse_resources[0]));
	    }

	/*
	 * Calculate the offset.  For the first resource, it's zero; for
	 * others, it's one after the prior entry's offset and length.
	 */
	if (parse_resources_length == 0)
		...
		offset = parse_resources[parse_resources_length - 1].offset
			+ parse_resources[parse_resources_length - 1].length
			+ 1;
	    }
...
parse_handle_v400_resources (sc_bool has_sound, sc_bool has_graphics)
{
	sc_vartype_t	vt_key, vt_value;
	sc_char		*file;
	sc_int32	length, offset;

	/*
	 * Retrieve the file and length for the sound just parsed.  If
	 * there's a file of non-zero length, rewrite its offset.
	 */
	if (has_sound)
	    {
		...
		/*
		 * If defined and has a length, rewrite the offset, and
		 * also the length in case changed.
		 */
		if (!sc_strempty (file) && length != 0)
		    {
			sc_int32	real_length;

			offset = parse_get_v400_resource_offset
						(file, length, &real_length);
...
parse_special (sc_char *special)
{
	...
	else if (!strcmp (special, "{V400_ROOM_EXIT:#Dest_#Var1_#Var2_#Var3}"))
	    {
		sc_int32		flag;
	...
	/* Parse room lists, with optional extra room. */
	else if (!strcmp (special, "{ROOM_LIST0}")
			|| !strcmp (special, "{ROOM_LIST1}"))
	    {
		sc_vartype_t	vt_key, vt_value;
		sc_int32	room_count, num_rooms, type, index;
		...
		switch (type)
			...
			vt_key.string = "Rooms";
			room_count = prop_get_integer
						(parse_bundle, "I<-s", &vt_key);

			if (!strcmp (special, "{ROOM_LIST1}"))
	...
	else if (!strcmp (special, "{OBJECT:#Parent}"))
	    {
		sc_vartype_t	vt_key;
		sc_int32	type;
	...
	else if (!strcmp (special, "{WALK:#Rooms_#Times}"))
	    {
		sc_vartype_t	vt_key, vt_value;
		sc_int32	num_stops, index;
	...
	/* Parse a room group variable size boolean list. */
	else if (!strcmp (special, "{ROOM_GROUP:[]BList}"))
	    {
		sc_vartype_t	vt_key, vt_value;
		sc_int32	num_rooms, index, l2index;
		sc_bool		in_group;
		...
		for (index = 0; index < num_rooms; index++)
			...
			if (in_group)
				...
				vt_key.string = "List2";
				parse_push_key (vt_key, KEY_STRING);
				vt_key.integer = l2index;
				parse_push_key (vt_key, KEY_INTEGER);
				vt_value.integer = index;
				parse_put_property (vt_value, PROP_INTEGER);
...
static void
parse_fixup_v390_v380_room_alt
		(sc_char *m1, sc_int32 type, sc_char *resource1,
		sc_char *m2, sc_int32 var2, sc_char *resource2,
		sc_int32 hide_objects, sc_char *changed,
		sc_int32 var3, sc_int32 display_room)
{
	sc_vartype_t	vt_key, vt_value, vt_rvalue, vt_gkey[2];
	sc_bool		has_sound, has_graphics;
	sc_int32	alt_count;
	sc_char		*soundfile1, *graphicfile1;
	sc_char		*soundfile2, *graphicfile2;

	/*
	 * Initialize resource files to empty, for cases where no resource
	 * is copied over from the main room (NULL resource1/2).
	 */
	soundfile1 = ""; graphicfile1 = "";
	soundfile2 = ""; graphicfile2 = "";
	...
	vt_key.string = "Alts";
	parse_push_key (vt_key, KEY_STRING);
	alt_count = (parse_get_property (&vt_rvalue, PROP_INTEGER))
					? vt_rvalue.integer : 0;
	parse_pop_key ();

	/*
	 * Lookup any resource details now, and save them.  Because this is
	 * not version 4.0, we can ignore lengths, and set them to zero when
	 * needed.
	 */
	...
	/*
	 * Create a room alt to match data passed in.  Start with the Alts
	 * string and the index to the alt being written.  To correctly
	 * emulate the parse, we also have to reverse the "Alts" and the
	 * index, as parse_put_property() will swap them.  Madness.
	 */
...
{
	sc_vartype_t	vt_key;
	sc_char		*m1, *m2, *changed;
	sc_int32	type, var2, hide_objects, var3, display_room;
	...
	/*
	 * Create a room alt to override all others, controlled by an
	...
	/*
	 * If a second task alternate description is defined, create a room
	 * alt to add after the main description, one that stops printing
	 * once done.
	 */
	...
	/*
	 * If still printing at this point, we need a catch-all room alt
	 * that will print.  So create one with an always true condition.
	 */
	type		= 0;			/* Task condition */
...
parse_fixup_v390 (sc_char *fixup)
{
	...
	else if (!strcmp (fixup, "|V390_TASK_ACTION:$Expr_#Var5|"))
	    {
		sc_vartype_t	vt_key;
		sc_int32	var2;
	 ...
	 * objects.
	 */
	else if (!strcmp (fixup, "|V390_OBJECT:_Openable_,Key|"))
	    {
		sc_vartype_t	vt_key, vt_value;
		sc_int32	openable;
	...
	/* Create a RestrMask that 'and's all the restrictions together. */
	else if (!strcmp (fixup, "|V390_TASK:$RestrMask|"))
	    {
		sc_vartype_t	vt_key, vt_value, vt_rvalue;
		sc_int32	restriction_count;

		/* Get a count of restrictions. */
		vt_key.string = "Restrictions";
		parse_push_key (vt_key, KEY_STRING);
		restriction_count = (parse_get_property
						(&vt_rvalue, PROP_INTEGER))
						? vt_rvalue.integer : 0;
		parse_pop_key ();
		...
		if (restriction_count > 0)
			...
			strcpy (restrmask, "#");
			for (index = 1; index < restriction_count; index++)
				strcat (restrmask, "A#");
	...
	/*
	 * Increment var1 for variable restrictions to compensate for there
	 * being no referenced text comparison (no string variables).
	 */
	else if (!strcmp (fixup, "|V390_TASK_RESTR:Var1>0?#Var1++|"))
	    {
		sc_vartype_t	vt_key, vt_value;
		sc_int32	var1;
	...
	else if (!strcmp (fixup, "|V390_ROOM:_Alts_|"))
	    {
		parse_fixup_v390_v380_room_alts ();
	    }
 ...
 * container capacity conversion factor and default object sizing, and the
 * count of task movements in a version 3.8 task.
 */
 ...
 */
static void
parse_fixup_v380_action (sc_int32 type, sc_uint16 var_count,
		sc_int32 var1, sc_int32 var2, sc_int32 var3)
{
	sc_vartype_t	vt_key, vt_value, vt_rvalue;
	sc_int32	action_count;

	/* Get a count of actions so far defined for the task. */
	vt_key.string = "Actions";
	parse_push_key (vt_key, KEY_STRING);
	action_count = (parse_get_property (&vt_rvalue, PROP_INTEGER))
					? vt_rvalue.integer : 0;
	parse_pop_key ();
 ...
 */
static void
parse_fixup_v380_movement (sc_int32 mvar1, sc_int32 mvar2, sc_int32 mvar3)
{
	sc_int32	var1;
	...
	/*
	 * Accept only player moves into rooms.  Other combinations, such
	 * as move player to worn by player, are unlikely.  And move player
	 * to same room as player isn't useful.
	 */
	if (mvar1 == 1)
	    {
		if (mvar3 == 0 && mvar2 >= 2)
	...
	/*
	 * Convert movement var1 into action var1.  Var1 is the dynamic
	 * object + 3, or 2 for referenced object, or 0 for all held.
	 */
	switch (mvar1)
	    {
	    case 2:	var1 = 2;		break;	/* Referenced obj */
	    case 3:	var1 = 0;		break;	/* All held */
	    default:	var1 = mvar1 - 1;	break;	/* Dynamic obj */
	    }
	...
	switch (mvar3)
	    ...
	    case 0:					/* To room */
		/*
		 * Convert movement var2 into action var2 and var3.  Var2 is
		 * 0 for move to room, 6 for move to player room.  Var3 is
		 * 0 for hidden, otherwise the room number plus one.
		 */
		if (mvar2 == 0)				/* Hidden */
		 ...
		 * a simple conversion, but check that var2 is not 'not
		 * selected' first.
		 */
		if (mvar2 > 0)
			parse_fixup_v380_action (0, 3, var1,
						mvar3 + 1, mvar2 - 1);
		break;
		...
		/*
		 * Convert movement var2 and var3 into action var3 and var2,
		 * in this case a simple conversion, since version 4.0 task
	    ...
	    default:
		sc_fatal ("parse_fixup_v380_movement:"
					" invalid mvar3, %ld\n", mvar3);
	    }
}
 ...
 */
static void
parse_fixup_v380_restr (sc_int32 type, sc_uint16 var_count,
		sc_int32 var1, sc_int32 var2, sc_int32 var3,
		sc_char *failmessage)
{
	sc_vartype_t	vt_key, vt_value, vt_rvalue;
	...
	vt_key.string = "Restrictions";
	parse_push_key (vt_key, KEY_STRING);
	restriction_count = (parse_get_property (&vt_rvalue, PROP_INTEGER))
					? vt_rvalue.integer : 0;
	parse_pop_key ();
 ...
 */
static void
parse_fixup_v380_obj_restr (sc_bool holding, sc_int32 holdobj,
		sc_char *failmessage)
{
	...
	if (holdobj > 0)
		...
		/*
		 * Create version 4.0 task restriction to check for either
		 * the referenced object or a dynamic object being either
		 * held or in the same room (visible to player).
		 */
...
static void
parse_fixup_v380_task_restr (sc_bool tasknotdone, sc_int32 task,
		sc_char *failmessage)
{
...
static void
parse_fixup_v380_wear_restr (sc_int32 wearobj, sc_char *failmessage)
{
	/* Ignore if no object selected. */
	if (wearobj > 0)
	    {
		sc_vartype_t	vt_key[3], vt_rvalue;
		sc_int32	object_count, object, dynamic;

		/*
		 * Create version 4.0 restrictions for something or nothing
		 * worn by player.
		 */
		if (wearobj == 1)
		    {
		...
		object_count = prop_get
				(parse_bundle, "I<-s", &vt_rvalue, vt_key)
					? vt_rvalue.integer : 0;
		...
		dynamic = 0;
		for (object = 0;
			object < object_count && wearobj >= 0; object++)
		    {
			sc_bool		bstatic, wearable;

			vt_key[1].integer = object;
			vt_key[2].string = "Static";
			bstatic = prop_get_boolean
					(parse_bundle, "B<-sis", vt_key);
			if (!bstatic)
			    {
				dynamic++;

				vt_key[2].string = "Wearable";
				wearable = prop_get_boolean
					(parse_bundle, "B<-sis", vt_key);
				if (wearable)
					wearobj--;
			    }
...
static void
parse_fixup_v380_npc_restr (sc_bool notinsameroom, sc_int32 npc,
		sc_char *failmessage)
{
	...
	if (npc > 0)
		...
		parse_fixup_v380_restr (3, 3, 0, var2, npc + 1, failmessage);
	    }
}

static void
parse_fixup_v380_objroom_restr (sc_int32 obj, sc_int32 objroom,
		sc_char *failmessage)
{
	/* Ignore if no object selected. */
	if (obj > 0)
...
static void
parse_fixup_v380_objstate_restr (sc_int32 obj, sc_int32 ivar1, sc_int32 ivar2,
		sc_char *failmessage)
{
	sc_vartype_t	vt_key[3], vt_rvalue;
	sc_int32	object_count, object, dynamic, var2, var3;
	...
	if (ivar1 == 3
		...
		for (object = 0; object <= obj - 1; object++)
			...
			openable = prop_get_integer
					(parse_bundle, "I<-sis", vt_key);
			if (openable > 0)
				stateful++;
		    }
		stateful--;
		...
		/*
		 * Create a version 4.0 restriction that checks that an
		 * object's state is open (var2 = 0) or closed (var2 = 1).
		 */
		var2 = (ivar1 == 3) ? 0 : 1;
		parse_fixup_v380_restr (1, 2, stateful + 1,
						var2, 0, failmessage);
		return;
	    }
	...
	/* Get the count of objects defined. */
	vt_key[0].string = "Objects";
	object_count = prop_get (parse_bundle, "I<-s", &vt_rvalue, vt_key)
					? vt_rvalue.integer : 0;
	...
	switch (ivar1)
	    {
	    case 1:	var2 = 4; var3 = ivar2;		break;	/* Inside */
	    case 2:	var2 = 5; var3 = ivar2;		break;	/* On */
	    case 5:	var2 = 1; var3 = ivar2 + 1;	break;	/* Held by */
	    case 6:	var2 = 2; var3 = ivar2 + 1;	break;	/* Worn by */
	    default:
		sc_fatal ("parse_fixup_v380_objstate_restr:"
					" invalid ivar1, %ld\n", ivar1);
	    }
...
parse_fixup_v380 (sc_char *fixup)
{
	...
	if (!strcmp (fixup, "|V380_OBJECT:#Capacity*10+2|"))
		...
		if (surfacecontainer == V380_OBJ_IS_CONTAINER)
			...
			capacity = capacity * V380_OBJ_CAPACITY_MULT
	...
	/*
	 * Exchange openable values 5 and 6, watch for a possible 1 from a
	 * 3.8 game (interpret as 0), and write -1 key for openable objects.
	 */
	else if (!strcmp (fixup, "|V380_OBJECT:_Openable_,Key|"))
	    {
		sc_vartype_t	vt_key, vt_value;
		sc_int32	openable;
	...
	/* Create version 4.0 task actions from a version 3.8 task. */
	else if (!strcmp (fixup, "|V380_TASK:_Actions_|"))
	    {
		sc_vartype_t	vt_key;
		sc_int32	score;
		...
		parse_push_key (vt_key, KEY_STRING);
		killsplayer = parse_get_integer_property ();
		parse_pop_key ();
		vt_key.string = "WinGame";
		parse_push_key (vt_key, KEY_STRING);
		wingame = parse_get_integer_property ();
		parse_pop_key ();
	...
	else if (!strcmp (fixup, "|V380_TASK:_Restrictions_|"))
	    {
		sc_vartype_t	vt_key, vt_value, vt_rvalue;
		sc_bool		holding, tasknotdone, notinsameroom;
		sc_int32	holdobj1, holdobj2, holdobj3, task;
		sc_int32	wearobj1, wearobj2, npc, obj1, obj1room, obj2;
		sc_char		*holdmsg, *taskmsg, *wearmsg, *companymsg;
		sc_char		*obj1msg;
		sc_int32	restriction_count;
	 ...
	 */
	else if (!strcmp (fixup, "|V380_OBJECT:_InitialPositions_|"))
	    {
		sc_vartype_t	vt_key[3], vt_rvalue;
		sc_int32	object_count, object, *object_type;

		/* Get a count of objects. */
		vt_key[0].string = "Objects";
		object_count = prop_get (parse_bundle, "I<-s",
				&vt_rvalue, vt_key) ? vt_rvalue.integer : 0;

		/* Build an array of object container/surface types. */
		object_type = sc_malloc (object_count * sizeof (*object_type));
		for (object = 0; object < object_count; object++)
		    {
			vt_key[1].integer = object;
			vt_key[2].string = "SurfaceContainer";
			object_type[object] = prop_get_integer
					(parse_bundle, "I<-sis", vt_key);
		    }
		...
		for (object = 0; object < object_count; object++)
			...
			if (initialposition > 2)
				...
				prop_put (parse_bundle,
						"I->sis", vt_value, vt_key);
			    }

			/*
			 * If initial position is on or in, decide which,
			 * depending on the type of the parent.  From this,
			 * expand initial position into a version 4.0 value.
			 */
			if (initialposition == 2)
			    {
				sc_int32	count, parent, index;

				/* Get parent container/surface index. */
				vt_key[1].integer = object;
				vt_key[2].string = "Parent";
				count = prop_get_integer
					(parse_bundle, "I<-sis", vt_key);

				/* Convert container/surface index. */
				for (parent = 0;
					parent < object_count && count >= 0;
					parent++)
				    {
					if (object_type[parent]
							== V380_OBJ_IS_CONTAINER
						|| object_type[parent]
							== V380_OBJ_IS_SURFACE)
						count--;
				    }
				...
				/*
				 * For both, adjust parent to be an object
				 * index for that type of object only.
				 */
				count = 0;
	...
	else if (!strcmp (fixup, "|V380_MaxSize_MaxWt_|"))
	    {
		sc_vartype_t	vt_key, vt_value;
		sc_int32	maxcarried;
	...
	else if (!strcmp (fixup, "|V380_GLOBAL:_MaxScore_|"))
	    {
		sc_vartype_t	vt_key[3], vt_value, vt_rvalue;
		sc_int32	task_count, maxscore, task;

		/* Get a count of tasks. */
		vt_key[0].string = "Tasks";
		task_count = prop_get (parse_bundle, "I<-s",
				&vt_rvalue, vt_key) ? vt_rvalue.integer : 0;
	...
	/* Convert walk meetobject from dynamic index to object. */
	else if (!strcmp (fixup, "|V380_WALK:_MeetObject_|"))
	    {
		sc_vartype_t	vt_key, vt_value, vt_rvalue, vt_gkey[3];
		sc_int32	meetobject, count, object_count, object;
		...
		object_count = prop_get (parse_bundle, "I<-s",
				&vt_rvalue, vt_gkey) ? vt_rvalue.integer : 0;

		/* Convert dynamic index to object, and rewrite. */
		count = meetobject - 1;
		for (object = 0;
			object < object_count && count >= 0; object++)
		    {
			sc_bool		bstatic;
	...
	else if (!strcmp (fixup, "|V380_ROOM:_Alts_|"))
	    {
		parse_fixup_v390_v380_room_alts ();
	    }
...
parse_fixup (sc_char *fixup)
{
	...
	switch (taf_get_version (parse_taf))
		...
		break;
	    case TAF_VERSION_390:	parse_fixup_v390 (fixup);	break;
	    case TAF_VERSION_380:	parse_fixup_v380 (fixup);	break;
	    default:
...
parse_element (sc_char *element)
{
	...
	switch (element[0])
	    {
	    case PARSE_ARRAY:		parse_array (element);		break;
	    case PARSE_VECTOR:		parse_vector (element, FALSE);	break;
	    case PARSE_VECTOR_ALTERNATE:parse_vector (element, TRUE);	break;
	    case PARSE_CLASS:		parse_class (element);		break;
	    case PARSE_EXPRESSION:	parse_expression (element);	break;
	    case PARSE_SPECIAL:		parse_special (element);	break;
	    case PARSE_FIXUP:		parse_fixup (element);		break;

	    case PARSE_INTEGER:			case PARSE_DEFAULT_ZERO:
	    case PARSE_BOOLEAN:			case PARSE_DEFAULT_TRUE:
	    case PARSE_DEFAULT_FALSE:
	    case PARSE_STRING:			case PARSE_DEFAULT_EMPTY:
	    case PARSE_IGNORE_INTEGER:		case PARSE_IGNORE_BOOLEAN:
	    case PARSE_IGNORE_STRING:
	    case PARSE_MULTILINE:	parse_terminal (element);	break;
	    default:
 ...
 */
static void
parse_descriptor (sc_char *descriptor)
{
	sc_char		*nextp;
 ...
 */
static void
parse_class (sc_char *aclass)
{
	sc_char			class[PARSE_TEMP_LENGTH];
	sc_uint16		index;

	/* Isolate the class name. */
	if (sscanf (aclass, "<%[^>]", class) != 1)
		sc_fatal ("parse_class: error in class, %s\n", aclass);
	if (parse_trace)
		sc_trace ("Parse: entering class %s\n", class);

	/* Find the class in the parse schema, and parse its descriptor. */
	for (index = 0; parse_schema[index].class != NULL; index++)
	    {
		if (!strcmp (parse_schema[index].class, class))
		    {
			sc_vartype_t	vt_key;

			/*
			 * Unless we are at the top level of the parse schema,
			 * push the class tag as a key.  The top level is
			 * "_GAME_", index 0, and isn't part of key formation.
			 */
			if (index > 0)
			    {
				vt_key.string = aclass + strlen (class) + 2;
				parse_push_key (vt_key, KEY_STRING);
			    }

			/* Parse each element in the descriptor. */
			parse_descriptor (parse_schema[index].descriptor);
	...
	/* Error in parse schema; class not found. */
...
parse_add_walkalerts (sc_prop_setref_t bundle)
{
	...
	sc_int32	npcs_count, npc;

	/* Get the count of NPCs. */
	vt_key[0].string = "NPCs";
	npcs_count = (prop_get (bundle, "I<-s", &vt_rvalue, vt_key))
					? vt_rvalue.integer : 0;
	...
	for (npc = 0; npc < npcs_count; npc++)
		...
		for (walk = 0; walk < walk_count; walk++)
			...
			if (starttask >= 0)
				...
				count = (prop_get (bundle,
						"I<-sis", &vt_rvalue, vt_key2))
						? vt_rvalue.integer : 0;
...
parse_add_movetimes (sc_prop_setref_t bundle)
{
	...
	for (npc = 0; npc < npcs_count; npc++)
		...
		for (walk = 0; walk < walk_count; walk++)
		    {
			sc_int32	waittimes;
			sc_int32	*movetimes, index;
			sc_vartype_t	vt_value;

			vt_key[3].integer = walk;
			vt_key[4].string = "Times";
			waittimes = (prop_get (bundle,
						"I<-sisis", &vt_rvalue, vt_key))
						? vt_rvalue.integer : 0;

			movetimes = sc_malloc ((waittimes + 1)
							* sizeof (*movetimes));
			memset (movetimes, 0, (waittimes + 1)
							* sizeof (*movetimes));
			for (index = waittimes - 1; index >= 0; index--)
			    {
				vt_key[4].string = "Times";
				vt_key[5].integer = index;
				movetimes[index] = prop_get_integer (bundle,
							"I<-sisisi", vt_key)
						+ movetimes[index + 1];
			    }
			movetimes[waittimes] = -2;

			for (index = 0; index <= waittimes; index++)
			    {
				vt_key[4].string = "MoveTimes";
				vt_key[5].integer = index;
				vt_value.integer = movetimes[index];
				prop_put (bundle,
						"I->sisisi", vt_value, vt_key);
			    }
			sc_free (movetimes);
		    }
	    }
...
{
	sc_vartype_t	vt_key[3], vt_rvalue;
	sc_int32	alr_count, index, alr;
	sc_int16	*alr_lengths, longest, shortest;
	sc_int16	length;
	...
	/*
	 * Set up an array of the lengths of ALR original strings,
	 * and while at it, get the shortest and longest defined.
	 */
	alr_lengths = sc_malloc (alr_count * sizeof (*alr_lengths));
	shortest = SHRT_MAX; longest = 0;
	...
	/*
	 * Now write a set of secondary properties that define the order
	 * of handling for ALRs.  Our friend qsort() can't help here as
	 * it doesn't define the final ordering of equal members, and
	 * we need here to retain file ordering for ALR originals of the
	 * same length.
	 */
...
parse_add_resources_offset (sc_prop_setref_t bundle, sc_tafref_t taf)
{
	...
	/*
	 * Get the resources offset from the TAF, or default to zero.  The
	 * resources offset is one byte after the end of game data.
	 */
	vt_key[0].string = "Globals";
...
parse_add_version (sc_prop_setref_t bundle, sc_tafref_t taf)
{
	...
	switch (taf_get_version (taf))
	    {
	    case TAF_VERSION_400:	vt_value.string = "4.00";	break;
	    case TAF_VERSION_390:	vt_value.string = "3.90";	break;
	    case TAF_VERSION_380:	vt_value.string = "3.80";	break;
	    default:
		sc_error ("parse_add_version_string:"
					" invalid TAF file version\n");
		vt_value.string = "[Unknown version]";			break;
	    }
 ...
 */
sc_bool
parse_game (sc_tafref_t taf, sc_prop_setref_t bundle)
{
	assert (taf != NULL);
	assert (bundle != NULL);
