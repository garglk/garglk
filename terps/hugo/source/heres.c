...
static int GetResourceParameters(char *filename, char *resname);
...
void DisplayPicture(void)
{
	...
	GetResourceParameters(filename, resname);
...
void PlayMusic(void)
{
	...
	if (!GetResourceParameters(filename, resname))
	{
		return;
	}
	...
	/* Check for MIDI */
	if (fseek(resource_file, resstart, SEEK_SET) == 0
	    && fread(resname, 4, 1, resource_file) == 1
	    && !memcmp(resname, "MThd", 4))
	{
	...
	/* Check for XM */
	if (fseek(resource_file, resstart, SEEK_SET) == 0
	    && fread(resname, 17, 1, resource_file) == 1
	    && !memcmp(resname, "Extended Module: ", 17))
	{
		resource_type = XM_R;
		goto Identified;
	}
	...
	/* Check for S3M */
	if (fseek(resource_file, resstart+0x2c, SEEK_SET) == 0
	    && fread(resname, 4, 1, resource_file) == 1
	    && !memcmp(resname, "SCRM", 4))
	{
		resource_type = S3M_R;
		goto Identified;
	}
	...
	if (fseek(resource_file, resstart+1080, SEEK_SET) == 0
	    && fread(resname, 4, 1, resource_file) == 1
	    && (!strcmp(resname+1, "CHN")   /* 4CHN, 6CHN, 8CHN */
...
void PlaySample(void)
{
	...
	if (fread(resname, 4, 1, resource_file) == 1 && !memcmp(resname, "WAVE", 4))
...
void PlayVideo(void)
{
	...
	if (fseek(resource_file, resstart, SEEK_SET) == 0
	    && fread(resname, 4, 1, resource_file) == 1
	    && resname[2]==0x01 && (unsigned char)resname[3]==0xba)
	{
	...
	/* Check for AVI */
	if (fseek(resource_file, resstart+8, SEEK_SET) == 0
	    && fread(resname, 4, 1, resource_file) == 1
	    && !memcmp(resname, "AVI ", 4))
	{
		resource_type = AVI_R;
		goto Identified;
	}
