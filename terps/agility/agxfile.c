...
static char *block_name[AGX_NUMBLOCK]={
...
int read_agx(fc_type fc, rbool diag)
{
  ...
  comblist=read_recblock(NULL,FT_SLIST,index[28].numrec,index[28].file_offset,
			 index[28].blocksize);

  num_prep=index[29].numrec;
  userprep=read_recblock(NULL,FT_SLIST,index[29].numrec,index[29].file_offset,
			 index[29].blocksize); 
  ...
  if (index[30].numrec != objextsize(0))
    fatal("Object flag block not of the correct size.");

  if (index[31].numrec != objextsize(1))
    fatal("Object property block not of the correct size.");  

  objflag=read_recblock(NULL,FT_BYTE,index[30].numrec,index[30].file_offset,
			index[30].blocksize);
  objprop=read_recblock(NULL,FT_INT32,index[31].numrec,index[31].file_offset,
			index[31].blocksize);			

  oflag_cnt=index[32].numrec;
  attrtable=read_recarray(NULL,sizeof(attrdef_rec),index[32].numrec,
  ...
  if (filehead.ext_own!='R' && filehead.fallback_ext<=1) { 
    ...
    for(i=0;i<oflag_cnt;i++)  /* These are converted later */
      attrtable[i].ystr=NULL; attrtable[i].nstr=NULL;
    for(i=0;i<oprop_cnt;i++)
      proptable[i].str_cnt=0;
    propstr_size=0; propstr=NULL;
    vartable=NULL; flagtable=NULL;
  } else { /* Normal case */	
    propstr_size=index[34].numrec;
    propstr=read_recblock(NULL,FT_STR,index[34].numrec,
			  index[34].file_offset,index[34].blocksize);
    
    if (index[35].numrec && index[35].numrec!=VAR_NUM+1) 
      fatal("AGX file corrupted: variable itemization table size mismatch.");
    vartable=read_recarray(NULL,sizeof(vardef_rec),index[35].numrec,
			   fi_varrec,"Variable Itemization Table",
			   index[35].file_offset, 
			   index[35].blocksize);

    if (index[36].numrec && index[36].numrec!=FLAG_NUM+1) 
      fatal("AGX file corrupted: flag itemization table size mismatch.");
    flagtable=read_recarray(NULL,sizeof(flagdef_rec),index[36].numrec,
  ...
  if (index[11].blocksize<=descr_maxmem) {
    /* ... if we decided to load descriptions into memory */
    mem_descr=read_recblock(NULL,FT_CHAR,index[11].numrec,
...
void agx_create(fc_type fc)
{
  ...
  bw_open(fc,fAGX);
  gindex=rmalloc(sizeof(index_rec)*AGX_NUMBLOCK);
