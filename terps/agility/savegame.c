/*  savegame.c-- Routines for saving and restoring games */
  ...
  if gs==NULL, we malloc a new block and return it */
{
  ...
  if (gs==NULL) {
    ...
    gs=rmalloc(state_size); /* This should be enough. */
    rm_trap=1;
...
/* Now do some simple consistancy checking on major variables */
  if (loc>maxroom || loc<0 || turncnt<0 || 
    ...
    writeln("Error: Save file inconsistent.");
    exit(EXIT_FAILURE);
  }
}
