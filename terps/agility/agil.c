...
/* This file contains the initialization and clean-up code
   as well as the interpreter's main loop. */
...
/* Overview of the source for AGiliTy:
 ...
 runverb.c:
  --standard verbs and the main routine for executing the player's action.

 object.c
  --Utility functions for manipulating game objects (nouns, creatures, etc.)
    including scope routines.
 
 metacommand.c
  --metacommand execution: scan the metacommands and execute them
  in the proper order. The actual execution of each individual command
  is handled in token.c.

 token.c
  --this module contains the actual code to run each of the (several hundered)
  different metacommands; called from metacommand.c.
  ...
  for(k=0;;k++) {
    ...
    if (buff[k]=='.' && PURE_DOT) 
	 ...
	 internal implementation details of the dictionary */
      punctuation=check_dot(in_text[ip],j,buff+k+1);
    else 
	...
	else if (input[ip]==0) input[ip]=ext_code[w_any]; /* _Real_ 'ANY' */
...
/*  lactor=lobj=lnoun=NULL;*/
  ...
  dbgflagptr=flag=rrealloc(flag,sizeof(rbool)*(FLAG_NUM+1)); 
...
void close_game(void)
{
 ...
 /* Now free everything in sight; this _shouldn't_ be necessary,
  ...
  rfree(agt_counter);rfree(agt_var);
  free_creat_fix();
  flag=rrealloc(flag,sizeof(rbool)); /* Preserve the debugging flag */
