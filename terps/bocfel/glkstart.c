...
int glkunix_startup_code(glkunix_startup_t *data)
{
  ...
  if(arg_status == ARG_HELP) return 1;
...
static void load_resources(void)
{
  ...
  if(file != NULL)
  {
    if(giblorb_set_resource_map(file) == giblorb_err_None) return;
  ...
  if(file != NULL) giblorb_set_resource_map(file);
#endif
}
