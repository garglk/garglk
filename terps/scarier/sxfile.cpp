...
typedef struct
{
  ...
  scr_int length;
  scr_bool is_open;
  scr_bool is_writable;
} sxr_scr_stream_t;
static sxr_scr_stream_t scr_serialization_stream = {NULL, 0, FALSE, FALSE};
 ...
 */
void *
file_open_file_callback (scr_bool is_save)
{
  sxr_scr_stream_t *const stream = &scr_serialization_stream;
...
scr_int
file_read_file_callback (void *opaque, scr_byte *buffer, scr_int length)
{
  sxr_scr_stream_t *const stream = (sxr_scr_stream_t *) opaque;
  scr_int bytes;
...
void
file_write_file_callback (void *opaque, const scr_byte *buffer, scr_int length)
{
  sxr_scr_stream_t *const stream = (sxr_scr_stream_t *) opaque;
  assert (opaque && buffer && length > 0);
  ...
  stream->data = (decltype(stream->data)) sxr_realloc (stream->data, stream->length + length);
