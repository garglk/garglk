...
{
  const scr_char *const program = argv[0];
  scr_bool is_verbose = FALSE, is_tracing = FALSE;
  const scr_char *trace_flags;
  sxr_test_descriptor_t *tests;
  scr_int count, index_, errors;
  assert (argc > 0 && argv);
  ...
  if (is_verbose)
    {
      sxr_trace ("--- %s Test Suite [Adrift %ld compatible]\n",
                scr_scarier_version (), scr_scarier_emulation ());
      if (argc < 2)
        return EXIT_SUCCESS;
    }
  else if (argc < 2)
  ...
  tests = (decltype(tests)) sxr_malloc ((argc - 1) * sizeof (*tests));
  ...
  for (index_ = 0; index_ < count; index_++)
    {
      ...
      scr_free_game (tests[index_].game);
    }
  sxr_free (tests);
