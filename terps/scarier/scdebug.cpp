...
static const scr_uint DEBUG_MAGIC = 0xc4584d2e;
...
typedef struct
{
  const scr_char *const command_string;
  const scr_command_t command;
} scr_strings_t;
static const scr_strings_t DEBUG_COMMANDS[] = {
 ...
 * disabling debugging.
 */
typedef struct scr_debugger_s
{
  scr_uint magic;
  scr_bool *watch_objects;
  scr_bool *watch_npcs;
  scr_bool *watch_events;
  scr_bool *watch_tasks;
  scr_bool *watch_variables;
  scr_bool watch_player;
  scr_bool single_step;
  scr_bool quit_pending;
  scr_uint elapsed_seconds;
} scr_debugger_t;
 ...
 */
static scr_bool
debug_is_valid (scr_debuggerref_t debug)
{
 ...
 */
static scr_debuggerref_t
debug_get_debugger (scr_gameref_t game)
{
 ...
 */
static scr_int
debug_variable_count (scr_gameref_t game)
{
  const scr_prop_setref_t bundle = gs_get_bundle (game);
  scr_vartype_t vt_key;
  scr_int variable_count;
 ...
 */
static void
debug_initialize (scr_gameref_t game)
{
  scr_debuggerref_t debug;

  /* Create the easy bits of the new debugging set. */
  debug = (decltype(debug)) scr_malloc (sizeof (*debug));
  ...
  debug->watch_objects = (decltype(debug->watch_objects)) scr_malloc (gs_object_count (game)
                                    * sizeof (*debug->watch_objects));
  debug->watch_npcs = (decltype(debug->watch_npcs)) scr_malloc (gs_npc_count (game)
                                 * sizeof (*debug->watch_npcs));
  debug->watch_events = (decltype(debug->watch_events)) scr_malloc (gs_event_count (game)
                                   * sizeof (*debug->watch_events));
  debug->watch_tasks = (decltype(debug->watch_tasks)) scr_malloc (gs_task_count (game)
                                  * sizeof (*debug->watch_tasks));
  debug->watch_variables = (decltype(debug->watch_variables)) scr_malloc (debug_variable_count (game)
 ...
 */
static void
debug_finalize (scr_gameref_t game)
{
  scr_debuggerref_t debug = debug_get_debugger (game);
  assert (debug_is_valid (debug));

  /* Free all allocated watchpoint arrays. */
  scr_free (debug->watch_objects);
  scr_free (debug->watch_npcs);
  scr_free (debug->watch_events);
  scr_free (debug->watch_tasks);
  scr_free (debug->watch_variables);
 ...
 */
static void
debug_help (scr_command_t topic)
{
 ...
 */
static void
debug_print_quoted (const scr_char *string)
{
...
static void
debug_print_task (scr_gameref_t game, scr_int task)
{
  const scr_prop_setref_t bundle = gs_get_bundle (game);
...
static void
debug_print_variable (scr_gameref_t game, scr_int variable)
{
  const scr_prop_setref_t bundle = gs_get_bundle (game);
 ...
 */
static void
debug_game (scr_gameref_t game, scr_command_type_t type)
{
  const scr_prop_setref_t bundle = gs_get_bundle (game);
  const scr_debuggerref_t debug = debug_get_debugger (game);
  scr_vartype_t vt_key[2];
  const scr_char *version, *gamename, *compiledate, *gameauthor;
  scr_int perspective, waitturns;
  scr_bool has_sound, has_graphics, has_battle;
  scr_char buffer[32];
  assert (debug_is_valid (debug));
 ...
 */
static void
debug_player (scr_gameref_t game,
              scr_command_t command, scr_command_type_t type)
{
  if (type != COMMAND_QUERY)
    {
 ...
 */
static scr_bool
debug_normalize_arguments (scr_command_type_t type,
                           scr_int *arg1, scr_int *arg2, scr_int limit)
{
  scr_int low = 0, high = 0;
  ...
  switch (type)
    ...
    case COMMAND_RANGE:
      ...
      break;
    default:
      scr_fatal ("debug_normalize_arguments: bad command type\n");
    }
 ...
 */
static scr_bool
debug_filter_room (scr_gameref_t game, scr_int room)
{
  return room == gs_playerroom (game);
}

static void
debug_dump_room (scr_gameref_t game, scr_int room)
{
  scr_int object, npc;
 ...
 */
static scr_bool
debug_filter_object (scr_gameref_t game, scr_int object)
{
  return obj_indirectly_in_room (game, object, gs_playerroom (game));
}

static void
debug_dump_object (scr_gameref_t game, scr_int object)
{
  const scr_prop_setref_t bundle = gs_get_bundle (game);
  scr_int openness;
  scr_vartype_t vt_key[3];
  scr_bool bstatic, is_statussed;
 ...
 */
static scr_bool
debug_filter_npc (scr_gameref_t game, scr_int npc)
{
  return npc_in_room (game, npc, gs_playerroom (game));
}

static void
debug_dump_npc (scr_gameref_t game, scr_int npc)
{
  debug_print_npc (game, npc);
  if_print_debug_character ('\n');
 ...
 */
static scr_bool
debug_filter_event (scr_gameref_t game, scr_int event)
{
  return gs_event_state (game, event) == ES_RUNNING;
}

static void
debug_dump_event (scr_gameref_t game, scr_int event)
{
  scr_char buffer[32];
 ...
 */
static scr_bool
debug_filter_task (scr_gameref_t game, scr_int task)
{
  return task_can_run_task (game, task);
}

static void
debug_dump_task (scr_gameref_t game, scr_int task)
{
 ...
 */
static void
debug_dump_variable (scr_gameref_t game, scr_int variable)
{
  const scr_prop_setref_t bundle = gs_get_bundle (game);
  const scr_var_setref_t vars = gs_get_vars (game);
  scr_vartype_t vt_rvalue;
 ...
 */
static void
debug_dump_common (scr_gameref_t game, scr_command_t command,
                   scr_command_type_t type, scr_int arg1, scr_int arg2)
{
  scr_int low = arg1, high = arg2;
  scr_int limit, index_;
  const scr_char *class_;
  scr_bool (*filter_function) (scr_gameref_t, scr_int);
  void (*dumper_function) (scr_gameref_t, scr_int);
  scr_bool printed = FALSE;
  ...
  switch (command)
    ...
    case DEBUG_OLDVARIABLES:
      ...
      break;
    default:
      scr_fatal ("debug_dump_common: invalid command\n");
    }
 ...
 */
static void
debug_buffer (scr_gameref_t game, scr_command_type_t type)
{
 ...
 */
static void
debug_print_resource (const scr_resource_t *resource)
{
  scr_char buffer[32];
 ...
 */
static void
debug_resources (scr_gameref_t game, scr_command_type_t type)
{
  scr_bool printed = FALSE;
 ...
 */
static void
debug_random (scr_command_type_t type, scr_int new_seed)
{
  const scr_char *random_type;
  scr_char buffer[32];
  ...
  random_type = scr_is_congruential_random () ? "congruential" : "platform";
 ...
 */
static void
debug_watchpoint_common (scr_gameref_t game, scr_command_t command,
                         scr_command_type_t type, scr_int arg1, scr_int arg2)
{
  const scr_debuggerref_t debug = debug_get_debugger (game);
  scr_int low = arg1, high = arg2;
  ...
  switch (command)
    ...
    case DEBUG_CLEARVARIABLES:
      ...
      break;
    default:
      scr_fatal ("debug_watchpoint_common: invalid command\n");
    }
 ...
 */
static void
debug_watchall_common (scr_gameref_t game,
                       scr_command_t command, scr_command_type_t type)
{
  const scr_debuggerref_t debug = debug_get_debugger (game);
  assert (debug_is_valid (debug));
 ...
 */
static scr_bool
debug_compare_variable (scr_gameref_t from, scr_gameref_t with, scr_int variable)
{
  ...
  if (!var_get (from_var, name, &var_type, &vt_rvalue)
    ...
    scr_fatal ("debug_compare_variable: can't find variable %s\n", name);
  else if (var_type != var_type2)
    scr_fatal ("debug_compare_variable: variable type mismatch %s\n", name);
  ...
  switch (var_type)
    ...
    case VAR_STRING:
      ...
      break;
    default:
      scr_fatal ("debug_compare_variable:"
 ...
 */
static scr_bool
debug_check_class (scr_gameref_t from, scr_gameref_t with,
                   const scr_char *class_, scr_int class_count,
                   const scr_bool *watchpoints,
                   scr_bool (*const compare_function)
                       (scr_gameref_t, scr_gameref_t, scr_int))
{
  scr_int index_;
  scr_bool triggered = FALSE;
 ...
 */
static scr_bool
debug_check_watchpoints (scr_gameref_t game)
{
  const scr_debuggerref_t debug = debug_get_debugger (game);
 ...
 * fails.
 */
static scr_command_t
debug_parse_command (const scr_char *command_string,
                     scr_command_type_t *type,
                     scr_int *arg1, scr_int *arg2, scr_command_t *help_topic)
{
  scr_command_t return_command;
  scr_command_type_t return_type;
  scr_int val1, val2, converted, matches;
  scr_char *help, *string, junk, wildcard;
  scr_bool is_help, is_parsed, is_wildcard;
  const scr_strings_t *entry;

  /* Allocate temporary strings long enough to take a copy of the input. */
  string = (decltype(string)) scr_malloc (strlen (command_string) + 1);
  help = (decltype(help)) scr_malloc (strlen (command_string) + 1);
  ...
  if (converted == 2 && scr_strcasecmp (help, "help") == 0)
    {
      is_help = TRUE;
      is_parsed = TRUE;
    }
  ...
  for (entry = DEBUG_COMMANDS; entry->command_string; entry++)
    {
      if (scr_strncasecmp (string, entry->command_string, strlen (string)) == 0)
        {
          matches++;
 ...
 */
static void
debug_dispatch (scr_gameref_t game,
                scr_command_t command, scr_command_type_t type,
                scr_int arg1, scr_int arg2, scr_command_t help_topic)
{
  ...
  switch (command)
    ...
    case DEBUG_NONE:
      break;
    default:
      scr_fatal ("debug_dispatch: invalid debug command\n");
    }
}
 ...
 */
static void
debug_dialog (scr_gameref_t game)
{
  const scr_filterref_t filter = gs_get_filter (game);
  const scr_debuggerref_t debug = debug_get_debugger (game);
  const scr_var_setref_t vars = gs_get_vars (game);
  ...
  while (TRUE)
    {
      scr_char buffer[DEBUG_BUFFER_SIZE];
      scr_command_t command, help_topic = DEBUG_NONE;
      scr_command_type_t type = COMMAND_QUERY;
      scr_int arg1 = 0, arg2 = 0;
 ...
 */
scr_bool
debug_run_command (scr_gameref_t game, const scr_char *debug_command)
{
  const scr_debuggerref_t debug = debug_get_debugger (game);
  scr_command_t command, help_topic;
  scr_command_type_t type;
  scr_int arg1, arg2;
 ...
 */
scr_bool
debug_cmd_debugger (scr_gameref_t game)
{
  const scr_debuggerref_t debug = debug_get_debugger (game);
 ...
 */
void
debug_game_started (scr_gameref_t game)
{
  const scr_debuggerref_t debug = debug_get_debugger (game);
...
void
debug_game_ended (scr_gameref_t game)
{
  const scr_debuggerref_t debug = debug_get_debugger (game);
 ...
 */
void
debug_turn_update (scr_gameref_t game)
{
  const scr_debuggerref_t debug = debug_get_debugger (game);
 ...
 */
void
debug_set_enabled (scr_gameref_t game, scr_bool enable)
{
  const scr_debuggerref_t debug = debug_get_debugger (game);
...
scr_bool
debug_get_enabled (scr_gameref_t game)
{
  const scr_debuggerref_t debug = debug_get_debugger (game);
