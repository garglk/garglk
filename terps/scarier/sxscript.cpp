 ...
 *   the game; those starting with '~' are sent to the SCARIER debugger.  Before
...
static const scr_int LINE_BUFFER_SIZE = 256;
static const scr_char NUL = '\0';
static const scr_char SCRIPT_COMMENT = '#';
static const scr_char GAME_COMMAND = '>';
static const scr_char DEBUG_COMMAND = '~';

/* Verbosity, and references to the game and script being processed. */
static scr_bool scr_is_verbose = FALSE;
static scr_game scr_loaded_game = NULL;
static sxr_script scr_script = NULL;

/* Script line number, and count of errors registered for the script. */
static scr_int scr_line_number = 0;
static scr_int scr_errors = 0;

/*
 * Current expected output, and game accumulated output, used by the
 * expectation checking function.
 */
static scr_char *scr_expectation = NULL;
static scr_char *scr_game_output = NULL;
 ...
 */
static void
scr_test_message (const scr_char *format, const scr_char *string)
{
  if (scr_is_verbose)
    {
      sxr_trace ("--- ");
      sxr_trace (format, string);
      sxr_trace ("\n");
    }
}

void
scr_test_failed (const scr_char *format, const scr_char *string)
{
  ...
  if (scr_is_verbose)
    {
      if (scr_line_number > 0)
        sxr_trace ("--- Near line %ld: ", scr_line_number);
      else
        sxr_trace ("--- ");
      sxr_trace (format, string);
      sxr_trace ("\n");
 ...
 */
static scr_bool
scr_is_line_type (const scr_char *line, scr_char type)
{
  return line[0] == type;
}

static scr_bool
scr_is_line_comment_or_empty (const scr_char *line)
{
...
static scr_bool
scr_is_line_game_command (const scr_char *line)
{
  return scr_is_line_type (line, GAME_COMMAND);
}

static scr_bool
scr_is_line_debug_command (const scr_char *line)
{
  return scr_is_line_type (line, DEBUG_COMMAND);
}

static scr_bool
scr_is_line_command (const scr_char *line)
{
  return scr_is_line_game_command (line) || scr_is_line_debug_command (line);
}

static scr_bool
scr_is_line_empty_debug_command (const scr_char *line)
{
...
typedef sxr_scr_location_t *sxr_scr_locationref_t;
 ...
 */
static void
scr_save_location (sxr_script script, sxr_scr_locationref_t location)
{
  fgetpos (script, &location->position);
  location->line_number = scr_line_number;
}

static void
scr_restore_location (sxr_script script, sxr_scr_locationref_t location)
{
  fsetpos (script, &location->position);
 ...
 */
static scr_char *
scr_get_next_line (sxr_script script)
{
  scr_char *buffer, *line = NULL;

  /* Allocate a buffer for line reads. */
  buffer = (decltype(buffer)) sxr_malloc (LINE_BUFFER_SIZE);

  /* Read until a significant line is found, or end of file or error. */
  while (fgets (buffer, LINE_BUFFER_SIZE, script))
    {
      scr_line_number++;
 ...
 */
static scr_char *
scr_concatenate (scr_char *string, const scr_char *buffer)
{
  /* If string is not null, concatenate buffer, otherwise duplicate. */
  if (string)
    {
      string = (decltype(string)) sxr_realloc (string,
    ...
    {
      string = (decltype(string)) sxr_malloc (strlen (buffer) + 1);
 ...
 */
static scr_bool
scr_get_next_section (sxr_script script,
                      scr_char **command, scr_char **expectation)
{
  scr_char *line, *first_line, *other_lines;
  sxr_scr_location_t location;
  ...
  if (first_line)
    {
      *command = sxr_normalize_string (first_line);
      *expectation = other_lines ? sxr_normalize_string (other_lines) : NULL;
      return TRUE;
    }
 ...
 */
static void
scr_expect (scr_char *expectation)
{
  /*
  ...
  if (expectation)
    {
      scr_expectation = (decltype(scr_expectation)) sxr_malloc (strlen (expectation) + 1);
      strcpy (scr_expectation, expectation);
      scr_game_output = (decltype(scr_game_output)) sxr_malloc (1);
      strcpy (scr_game_output, "");
    }
  else
    {
      sxr_free (scr_expectation);
      scr_expectation = NULL;
      sxr_free (scr_game_output);
...
scr_verify_expectation (void)
{
  ...
  if (scr_expectation && scr_game_output)
    {
      scr_game_output = sxr_normalize_string (scr_game_output);
 ...
 */
static void
scr_execute_debugger_command (const scr_char *command, scr_char *expectation)
{
  scr_bool status;
   ...
   */
  scr_set_game_debugger_enabled (scr_loaded_game, TRUE);
  status = scr_run_game_debugger_command (scr_loaded_game, command + 1);
 ...
 */
static scr_bool
scr_read_line_callback (scr_char *buffer, scr_int length)
{
  scr_char *command, *expectation;
  ...
  if (scr_get_next_section (scr_script, &command, &expectation))
    {
      if (scr_is_line_debug_command (command))
        ...
        scr_set_game_debugger_enabled (scr_loaded_game, FALSE);
 ...
 */
static void
scr_print_string_callback (const scr_char *string)
{
  assert (string);

  if (scr_game_output)
    {
      scr_game_output = (decltype(scr_game_output)) sxr_realloc (scr_game_output,
 ...
 */
void
scr_start_script (scr_game game, sxr_script script)
{
  scr_char *command, *expectation;
  sxr_scr_location_t location;
  assert (game && script);
  ...
  assert (!scr_loaded_game && !scr_script);
  scr_loaded_game = game;
  scr_script = script;
  scr_line_number = 0;
  scr_errors = 0;
