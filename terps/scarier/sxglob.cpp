...
typedef struct
{
  const scr_char *const pattern;
  const scr_char *const string;
} sxr_test_data_t;

static const sxr_test_data_t SHOULD_MATCH[] = {
  {"a", "a"}, {"abc", "abc"}, {"", ""},
  {"*", ""}, {"*", "abc"}, {"*", "cba"},
  {"*c", "c"}, {"*c", "abc"}, {"*c", "cbac"},
  {"a*", "a"}, {"a*", "abc"}, {"a*", "abca"},
...
static const sxr_test_data_t SHOULD_NOT_MATCH[] = {
  {"a", "b"}, {"abc", "abd"}, {"a", ""}, {"", "a"},
  {"*c", "a"}, {"*c", "ab"}, {"*c", "abca"},
  {"a*", "c"}, {"a*", "cba"}, {"a*", "cbac"},
...
glob_self_test (void)
{
   ...
   * trust that sxr_fatal() calls abort() (though it should).
   */
  if (errors > 0)
    {
      sxr_fatal ("glob_self_test: %ld self-test error%s found, aborting\n",
                errors, (errors == 1) ? "" : "s");
      abort ();
    }
}
 ...
 */
scr_bool
glob_match (const scr_char *pattern, const scr_char *string)
{
