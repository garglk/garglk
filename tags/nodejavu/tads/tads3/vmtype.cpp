...
int vm_val_t::equals(VMG_ const vm_val_t *val, int depth) const
{
    ...
    case VM_CODEPTR:
         ...
         */
        return (val->typ == typ && val->val.ptr == this->val.ptr);

    case VM_OBJ:
        /* use the object's polymorphic equality test routine */
        return vm_objp(vmg_ this->val.obj)
            ->equals(vmg_ this->val.obj, val, depth);

    case VM_PROP:
        /* we match if the other value is the same property ID */
        return (val->typ == VM_PROP && val->val.prop == this->val.prop);

    case VM_INT:
        /* we match if the other value is the same integer */
        return (val->typ == VM_INT && val->val.intval == this->val.intval);

    case VM_ENUM:
        return (val->typ == VM_ENUM && val->val.enumval == this->val.enumval);

    case VM_SSTRING:
        /* use the standard string comparison routine */
        return CVmObjString::
            const_equals(vmg_ G_const_pool->get_ptr(this->val.ofs), val);

    case VM_LIST:
        /* use the standard list comparison routine */
        return CVmObjList::
            const_equals(vmg_ this,
                         G_const_pool->get_ptr(this->val.ofs), val, depth);

    case VM_CODEOFS:
    case VM_FUNCPTR:
        /* we match if the other value is the same code offset */
        return (val->typ == typ && val->val.ofs == this->val.ofs);
...
int vm_val_t::gen_compare_to(VMG_ const vm_val_t *val) const
{
    ...
    case VM_OBJ:
        ...
        return vm_objp(vmg_ this->val.obj)
            ->compare_to(vmg_ this->val.obj, val);

    case VM_SSTRING:
        /* compare the string */
        return CVmObjString::
            const_compare(vmg_ G_const_pool->get_ptr(this->val.ofs), val);
 ...
 */
void vm_val_t::set_datatype(VMG_ const vm_val_t *val)
{
    /* check for an object */
    if (val->typ == VM_OBJ)
    {
        /* check for List and String values */
        if (val->get_as_string(vmg0_) != 0)
        {
            /* treat this the same as a string constant */
            set_int((int)VM_SSTRING);
        }
        else if (val->get_as_list(vmg0_) != 0)
        {
