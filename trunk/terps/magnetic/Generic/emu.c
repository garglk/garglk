...
type16 code1 = 0x05, code2 = 0x09, v4_id = 0, next_table = 1;
...
const type16s actions[16] =
{0x09B4,0x0073,0x09C0,0x09C6, /*Wonderland*/
 0x0136,0x0060,0x0142,0x0148, /*Corruption*/
 0x09B0,0x006C,0x09BC,0x09C2, /*Fish*/
 0x0976,0x006C,0x0982,0x0988  /*Guild*/};
...
type8 ms_init(type8s * name, type8s * gfxname, type8s * hntname)
{
	FILE *fp;
...
void do_line_a(void)
{
	type8 l1c;
	type8s *str;
	type16 ptr, ptr2, tmp16, d0reg, d2reg, d5reg;
	type32 a1reg, tmp32;
 ...
 */
	...
	if ((byte2 < 0xdd) || (version < 4 && byte2 < 0xe4) || (version < 2 && byte2 < 0xed))
	{
		...
		case 2:	/* A0DF */
			...
			d0reg = (type16)read_reg(0,2);
			d2reg = (type16)read_reg(2,2);
			d5reg = (type16)read_reg(5,2);
...
#ifdef LOGGFX
/*
				out2("table base: %d Table offset: %d\n",gfxtable,table_dist);
*/
#endif
			}
#ifdef LOGGFX
/*
			out2("Entering DF routine with code d0=%8.8x d2=%8.8x a3=%8.8x\n",d0reg,d2reg,(type16)code[a1reg]);
*/
#endif
			if ((d0reg == 2) && (version == 4))
			{
				type16s action = -1;
				for (tmp16 = 0; tmp16 < 4; tmp16++)
				{
					if (d5reg == actions[(v4_id<<2)+tmp16])
						action = tmp16;
				}
			...
			{
				if ((d2reg >= gfxtable) && (d2reg < 0xffff) &&
				   (((type16)code[a1reg] >= code1) && ((type16)code[a1reg] <= code2) &&
				   ((d0reg >= code1) && (d0reg <= code2))))
				{
					if (code[table_dist+(d2reg-gfxtable)-1] == 0x00)
					{
#ifdef LOGGFX
						out2("PICTURE IS %s\n", code + a1reg + 3);
#endif
